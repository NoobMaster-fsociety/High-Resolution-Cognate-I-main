<?php
    ...
    || $data->Customer_payment == "" || $data->Customer_date == null|| $data->Customer_Quantity== "")
    {
    ...
    //    echo date('Y-m-d', strtotime($date));
        ...
        if ($item->Check_CustomerAuthentication())
        {
            echo json_encode(array(array('Result' => $item->create_customerdata(),'Message' => null)));

            $Converted_purchase = explode("," ,$data->Customer_purchase);
            $converted_Quantity = explode("," ,$data->Customer_Quantity);

             
            for ($i = 0; $i < count($converted_Quantity); $i++)
            {
          
              $item-> update_newStocks(str_replace(' ','+',$Converted_purchase[$i]),$converted_Quantity[$i]);
            }


        }else{
            echo json_encode(array(array('Result' => $item->Check_CustomerAuthentication(), 'Message' => "Data exists.")));
        }
